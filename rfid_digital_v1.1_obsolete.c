...
uint8_t rfid_buffer [RFID_BUFFER_SIZE];  //½óôåð RFID
...
uint8_t rfid_read(uint8_t* data)
{
	...
	if(rfid_decode() != RFID_OK) return RFID_PARITY_ERR; //çàíèìàåò äî 3ìñ
	if(rfid_check() != RFID_OK) return RFID_PARITY_ERR; //çàíèìàåò 50ìêñ
